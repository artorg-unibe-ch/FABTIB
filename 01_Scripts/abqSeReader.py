              ...
              if ('S' in allFields):
                ...
                for stressValue in stressSet.values:
                  if stressValue.position == INTEGRATION_POINT : 
                    ...
                    S_sum['S11'] += stressValue.data[0]*ivols[elId][ipId]
                    S_sum['S22'] += stressValue.data[1]*ivols[elId][ipId]
                    S_sum['S33'] += stressValue.data[2]*ivols[elId][ipId]
                    S_sum['S12'] += stressValue.data[3]*ivols[elId][ipId]
                    S_sum['S13'] += stressValue.data[4]*ivols[elId][ipId]
                    S_sum['S23'] += stressValue.data[5]*ivols[elId][ipId]
                    nS+=1
              ...
              if ('E' in allFields):
                ...
                for strainValue in strainSet.values:
                  if strainValue.position == INTEGRATION_POINT : 
                    elId = strainValue.elementLabel
                    ipId = strainValue.integrationPoint
                    E_sum['E11'] += strainValue.data[0]*ivols[elId][ipId]
                    E_sum['E22'] += strainValue.data[1]*ivols[elId][ipId]
                    E_sum['E33'] += strainValue.data[2]*ivols[elId][ipId]
                    E_sum['E12'] += strainValue.data[3]*ivols[elId][ipId]
                    E_sum['E13'] += strainValue.data[4]*ivols[elId][ipId]
                    E_sum['E23'] += strainValue.data[5]*ivols[elId][ipId]
                    nE+=1
 
        for Sij in S_sum : 
          S_sum[Sij] = S_sum[Sij]/volume
        S_step[step.name] = S_sum

        for Eij in E_sum : 
          E_sum[Eij] = E_sum[Eij]/volume
